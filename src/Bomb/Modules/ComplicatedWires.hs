...
wireInstruction w =
  case w of Wire True True True True -> DoNotCut
...
instructionToAction i = do
  bats <- gets batteries
  parallel <- gets parallel
  even <- gets serialEven
  case i of Cut -> output "Cut the wire"
            DoNotCut -> output "Do NOT cut the wire"
            ParallelPort -> case parallel of Nothing -> askParallel (output "Cut the wire") (output "Do NOT cut the wire")
                                             Just True -> output "Cut the wire"
                                             Just False -> output "Do NOT cut the wire"
            SerialEven -> case even of Nothing -> askSerialEven (output "Cut the wire") (output "Do NOT cut the wire")
                                       Just True -> output "Cut the wire"
                                       Just False -> output "Do NOT cut the wire"
